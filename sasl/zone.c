...
zone_header *zone_new(unsigned size, struct zone_header *parent)
{
  ...
  if (z==NULL) {
    (void) err_zone("zone_new: malloc out of space");
    return 0; /*NOTREACHED*/
  }
...
int zone_check_do(pointer root, pointer defs, pointer freelist)
{
  ...
  /* zone check */
  {
    ...
    if ((refc_inuse_count + refc_free_count) > zone_total_size)
      return refc_check_log2("!!(refc_inuse_count + refc+free+count) > zone_total_size: %d > %d", 5, (refc_inuse_count + refc_free_count), zone_total_size);
      
    if ((refc_inuse_count + refc_free_count) != total_used)
      return refc_check_log2("!!refc_inuse_count + refc+free+count) != (zone_total_used): %d > %d", 5, (refc_inuse_count + refc_free_count), total_used);
